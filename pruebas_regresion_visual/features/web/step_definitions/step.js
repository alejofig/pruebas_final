...
Given(
  "I fill the input with id selector {string} with random email",
...
Given(
  "I fill the input with id selector {string} with {string}",
  async function (inputId, email) {
...
Given(
  "I select the option {string} in the invite a new staff modal",
  async function (target) {
    let elements = await this.driver.$$(
      ".gh-roles-container .gh-radio .gh-radio-label"
    );
    const filtered = await searchableElementIncluding(
      this.driver,
      elements,
      target
    );
...
Given(
  "I see an error saying {string} in the {string} field",
  async function (response, field) {
...
Given(
  "I fill the post title with {string} and description with {string}",
...
Then(
  "I Check post has title {string} and description {string}",
  async function (title, description) {
...
Given("I select existing tag", async function () {
  let element = await this.driver.$(
    "a.ember-view.gh-list-data.gh-tag-list-title"
  );
  element.click();
  await delay(3000);
});
...
Then(
  "I chek tag with random name exists and has description",
  async function () {
    const titles = await this.driver.$$("h3.gh-tag-list-name");
    let filtered = await searchableElementIncluding(
      this.driver,
      titles,
      returns["randomTagTitle"]
    );
    expect(filtered.length).to.equal(1);
    ...
    filtered = await searchableElementIncluding(
      this.driver,
      descriptions,
      returns["randomTagDescription"]
    );
    expect(await filtered[0].getText()).to.equal(
      returns["randomTagDescription"]
    );
...
Given("I am logged in the admin section", async function () {
  ...
  expect(currentUrl).equal(
    "http://uniandes.ingenio.com.co:2368/ghost/#/dashboard"
  );
});
...
When("I go to my profile", async function () {
  ...
  await element.click();
  let secondElement = await this.driver.$(
    'li a[href="#/settings/staff/johnattan/"]'
  );
...
When(
  "I add a menu to {string} with label {string} as a new element in {string} navigation",
  async function (url, label, navigation) {
    const navigationLinks = {
      primary: "#settings-navigation",
      secondary: "#secondary-navigation",
    };

    const elementLabel = this.driver.$(
      navigationLinks[navigation] +
        " .gh-blognav-item:not(.gh-blognav-item--sortable) input[placeholder='Label']"
    );
    await elementLabel.setValue(label);

    const elementURL = this.driver.$(
      navigationLinks[navigation] +
        " .gh-blognav-item:not(.gh-blognav-item--sortable) input:not([placeholder='Label'])"
    );
...
When(
  "I check the last menu item in {string} navigation",
  async function (navigation) {
    const navigationLinks = {
      primary: ".gh-head-menu",
      secondary: ".gh-foot-menu",
    };
    const elementsURL = await this.driver.$$(
      navigationLinks[navigation] + " .nav li:last-child a"
    );
    const menuElements = await this.driver.$$(
      navigationLinks[navigation] + " .nav li"
    );
    ...
    for (const elementURL of elementsURL) {
      try {
        if (typeof elementURL === "object") {
          elementsHrefs.push(
            await this.driver.$(elementURL).getAttribute("href")
          );
        }
      } catch (e) {}
    }

    returns["last-item-link"] = {
...
When(
  "I remove the last menu item in {string} navigation",
  async function (navigation) {
    const navigationLinks = {
      primary: "#settings-navigation",
      secondary: "#secondary-navigation",
    ...
    const elementLabel = this.driver.$(
      navigationLinks[navigation] +
        " .sortable-objects .draggable-object:last-child button.gh-blognav-delete"
    );
...
When(
  "I check that the last menu item was deleted in the {string} navigation",
  async function (navigation) {
    const navigationLinks = {
      primary: ".gh-head-menu",
      secondary: ".gh-foot-menu",
    ...
    const elementsURL = await this.driver.$$(
      navigationLinks[navigation] + " .nav li a"
    );
    const menuElements = await this.driver.$$(
      navigationLinks[navigation] + " .nav li"
    );
    ...
    for (let i = 0; i < elementsURL.length; i++) {
      try {
        if (typeof elementsURL[i] === "object") {
          expect(returns["last-item-link"].elementsHrefs[i]).to.equal(
            await this.driver.$(elementsURL[i]).getAttribute("href")
          );
        }
      } catch (e) {}
    }

    expect(menuElements.length).to.equal(
      returns["last-item-link"].totalElements - 1
    );
    return await delay(5000);
  }
);
...
When("I go into {string} settings", async function (settings) {
  const element = await this.driver.$(
    '.gh-main a[href="#/settings/' + settings + '/"]'
  );
  await element.click();
  return await delay(5000);
});
...
When(
  "I fill the input in the position {int} in the expanded section",
...
Then(
  "I check that the full name is {string}",
  async function (newFullNameProfile) {
...
When(
  "I enter {string} to the input with class {string}",
  async function (value, inputClass) {
...
Given(
  "I fill the input with id selector name {string} with {string}",
  async function (inputId, name) {
...
Given(
  "I fill the input with id selector description {string} with {string}",
  async function (inputId, description) {
...
Given(
  "I fill the page title with {string} and description with {string}",
  async function (title, description) {
    let textArea = await this.driver.$("textarea.gh-editor-title");
    await textArea.click();
    await textArea.setValue(title);
...
Then(
  "I Check page has title {string} and description {string}",
  async function (title, description) {
    let titleHtml = await this.driver.$("h1.single-title");
    let descHtml = await this.driver.$("div.single-content p");
    expect(await titleHtml.getText()).equal(title);
...
Then(
  "I take a snapshot {string} step, {string} feature",
  async function (step_name, feature_name) {
    const fs = require("fs");
    await this.driver.takeScreenshot().then(function (data) {
      var base64Data = data.replace(/^data:image\/png;base64,/, "");
      fs.mkdirSync(`images/kraken/${feature_name}`, { recursive: true });
      fs.writeFile(
        `images/kraken/${feature_name}/${step_name}.png`,
        base64Data,
        "base64",
        function (err) {
